...
plot <- ggplot(score_summary, aes(n, score_mean)) +
     geom_point(aes(color = scored_by_mean, size = scored_by_mean), alpha = 0.7,
                show.legend = FALSE) +
     geom_smooth(se = FALSE, color = "darkgray") +
     scale_x_log10() +
     scale_color_viridis(option = "A") +
     labs(title = "Do Bigger Studios Make Higher Rated Anime?",
          subtitle = "Point size and color correlates with number of user ratings",
